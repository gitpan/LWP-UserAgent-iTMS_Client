...
sub new {
    ...
    $self->{protocol}->{login_id} = $self->{protocol}->{user_id}
      || '?? unknown ??';
    ...
    if ( $response->is_success ) {
        ...
        if ( $encoding =~ /x-aes-cbc/ ) {
            ...
            if ($h_twofish) {
                ...
                $self->err(
                    "Bad encoding protocol in response from $url$params");
            }
            if ($h_iviv) {
                my $alg =
                  new Crypt::Rijndael( $key, Crypt::Rijndael::MODE_CBC );
                $alg->set_iv( hexToUchar($h_iviv) );
                $response->content( $alg->decrypt( $response->content ) );
            }
            else {
                $self->err(
                    "No aes crypto-iv given in response from $url$params");
            }
        }
        if ( $encoding =~ /gzip/ ) {
            $response->content(
...
sub login {
    ...
    $self->err("Need user_id and password in call to login for iTMS_Client")
      unless $self->{protocol}->{user_id}
      and $self->{protocol}->{password};
    ...
    $self->err("Login failure! Message: $customer_message")
      unless $jingleDocType
      and $jingleDocType =~ /Success$/i;
...
sub purchase {
    ...
    my $buy_request_params = '?'
      . $buy_params
      . '&creditBalance='
      . $self->{protocol}->{credit_balance}
      . '&creditDisplay='
      . uri_escape( $self->{protocol}->{credit_display} )
      . '&freeSongBalance='
      . $self->{protocol}->{free_song_balance}
      . '&guid='
      . $self->gu_id
...
sub download_songs {
    my ($self) = @_;
    while ( my ( $downloadKey, $info ) =
        each %{ $self->{protocol}->{downloadable} } )
    {
        ...
        my $path =
            $self->{protocol}->{download_dir} . $sep
...
sub pending_downloads {
    ...
    if ( $response->is_success ) {
        my $dicts = parse_dict( $response->content, 'array/dict' );
...
sub preview {
    ...
    if ( $preview->is_success ) {
        ...
        my $new_fh = $self->open_new_pathname( $path, $preview_name ) or next;
        binmode $new_fh;
...
sub drms_dir {
    my ($self) = @_;
    return sprintf( "%s%s%sdrms%s",
        $self->{protocol}->{home_dir},  $self->{protocol}->{path_sep},
        $self->{protocol}->{maybe_dot}, $self->{protocol}->{path_sep} );
}
...
sub get_saved_keys {
    ...
    opendir( my $dh, $drms_dir )
      or $self->err("Cannot open DRMS directory: $!");
...
# XML parsing of 'dict' content in XML data
sub parse_dict {
    my ( $content, $path ) = @_;
    my @entries;
    my $entry_index = -1;
